...
exports.loginRequest = catchAsyncError(async (req, res, next) => {
  ...
  const searchUserSql = `SELECT * FROM admins WHERE email = ? `;
...
exports.loginVerify = catchAsyncError(async (req, res, next) => {
  ...
  if (otpRes) {
    const searchUserSql = `SELECT * FROM admins WHERE admin_mobile = ? `;
    let searchResult = await db(searchUserSql, phone);
...
exports.signUp = catchAsyncError(async (req, res, next) => {
  ...
  const searchUserSql = `SELECT * FROM admins WHERE admin_mobile = ? OR email = ?`;
  ...
  await generateInsertStatement("admins", req);
  res.status(200).json({
    success: "true",
  });
});
